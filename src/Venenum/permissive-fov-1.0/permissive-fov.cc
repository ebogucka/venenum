...
namespace
{
  ...
  unsigned int * allocateMask(int width, int MAP_HEIGHT)
  {
    int cellCount = width * MAP_HEIGHT;
                                    ...
                                    int south, int east, int west)
{
  ...
  mask->MAP_HEIGHT = mask->south + 1 + mask->north;
  mask->mask = allocateMask(mask->width, mask->MAP_HEIGHT);
  if (mask->mask == NULL)
  {
                                    ...
                                    char const * fileName)
{
  ...
  mask->MAP_HEIGHT = static_cast<int>(input.size());
  mask->mask = allocateMask(mask->width, mask->MAP_HEIGHT);
  // TODO: Out of memory
  list<string>::iterator inputPos = input.begin();
  unsigned int * intPos = mask->mask;
  int bitPos = 0;
  for (int i = 0; i < mask->MAP_HEIGHT; ++i, ++inputPos)
  {
    for (int j = 0; j < mask->width; ++j)
    {
      ...
      case '@':
        ...
        mask->south = i;
        mask->west = j;
        mask->north = mask->MAP_HEIGHT - 1 - mask->south;
